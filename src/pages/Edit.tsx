...
import { Save, ArrowLeft, Settings, Home, Sparkles } from 'lucide-react'
...
const AiPanel = lazy(() => import('@/components/ai/Panel'))
...
const Edit: React.FC = () => {
  ...
  const handleApplyAiTags = useCallback(
    (tags: string[]) => {
      ...
      setNote((prev) => (prev ? { ...prev, tags: merged } : null))
    },
    [note]
  )

  const checkRemoteConflict = useCallback(async (): Promise<boolean> => {
    if (!note?.id || isNewNote || !navigator.onLine) return false
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let savedId = note.id
      let savedCreatedAt = note.createdAt
      const savedUpdatedAt = new Date().toISOString()
      const creating = isNewNote

      // 必须等后端入库（并校验可读）成功后，才提示保存成功
      if (creating) {
        ...
        await notesApi.updateNote(note.id, noteData, { remoteOnly: true })
    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="text-center">
          <h2 className="mb-4 font-bold text-gray-900" style={{ fontSize: '1.25rem' }}>
            笔记不存在
          </h2>
  ...
  return (
    <div
      className="flex min-h-screen flex-col bg-gradient-to-br from-gray-100/60 to-gray-200/60 md:h-screen md:overflow-hidden"
      style={{
        backgroundImage: "var(--app-bg-image, url('/background.webp'))",
        backgroundSize: 'cover',
        backgroundPosition: 'center',
    ...
    >
      <header className="shrink-0 border-b border-white/30 bg-white/30 shadow-sm backdrop-blur-md">
        <div className="w-full">
          <div className="flex h-16 items-center">
            <Button onClick={handleBack} variant="primary">
              <ArrowLeft className="mr-2 size-4" />
            ...
            <div className="flex items-center space-x-4">
              <Button onClick={() => setIsAiOpen(true)} variant="primary">
      ...
      <main className="flex min-h-0 w-full flex-1 flex-col">
        <div className="flex min-h-0 w-full flex-1 flex-col">
          {conflictWarning && (
            <div className="shrink-0 rounded-md border border-amber-200 bg-amber-50 p-4">
              <div className="text-amber-800">{conflictWarning}</div>
            </div>
          )}

          {error && (
            <div className="shrink-0 rounded-md border border-red-200 bg-red-50 p-4">
          ...
          <div
            data-layout-card
            className="flex min-h-0 flex-1 flex-col border-y border-white/30 bg-white/30 shadow backdrop-blur-md md:flex-row md:items-stretch md:overflow-hidden"
            data-edit-row
          >
            <EditorToolbar embedded />

            <div className="min-h-0 min-w-0 flex-1 overflow-y-auto overscroll-contain md:border-l md:border-white/30">
              {contentLoading ? (
