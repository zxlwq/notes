...
export default async function handler(req, res) {
  ...
  if (req.method !== 'GET' && !(await checkAuth(req, pool))) {
  ...
  try {
    ...
    if (!isAllowedUiSettingKey(key)) {
      return res.status(400).json({ success: false, error: 'Unsupported setting key' })
    }
    ...
    await appendLog('error', 'Failed to process settings API', { error: String(e) })
