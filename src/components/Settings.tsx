...
import { getEncryptionPassword } from '@/lib/crypto'
import {
  buildImportPreview,
  fetchNotesForExport,
  notesToMarkdown,
...
import ThemePicker from '@/components/settings/Theme'
import { persistTheme, resolveTheme } from '@/lib/theme'
...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  const nestedModalOpen =
    modal.alertState.isOpen ||
    modal.confirmState.isOpen ||
    modal.promptState.isOpen ||
    modal.selectState.isOpen ||
    isUploadModalOpen ||
    logsOpen ||
    recoveryCode !== null
  useFocusTrap(isOpen && !nestedModalOpen, settingsDialogRef)
  useEscapeClose(isOpen && !nestedModalOpen, onClose)
    ...
    if (savedSettings) {
      try {
        ...
        setSettings({
          ...defaultSettings,
          ...parsedSettings,
          theme: resolveTheme(parsedSettings.theme),
        })
      } catch (error) {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      modal.showAlert(
        '管理员密码已修改，笔记加密密钥不变。系统将在3秒后退出，请用新密码重新登录。',
        {
          type: 'success',
          title: '修改成功',
          confirmText: '确定',
