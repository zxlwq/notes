...
import { ensureSharesTable, revokeSharesByNoteId } from '../../../shared/neon-shares.js'
...
export default async function onRequest(context) {
  ...
  try {
    ...
    if (method === 'PUT') {
      ...
      try {
        const updated = await updateNote(sql, noteId, { title, content, tags })
        if (!updated) {
    ...
    if (method === 'DELETE') {
      ...
      try {
        ...
        await revokeSharesByNoteId(sql, noteId)
