...
const Editor: React.FC<EditorProps> = ({
  ...
  const bindCodemirror = useCallback((cm: CodeMirrorInstance) => {
    cmRef.current = cm
    // 使用 markdown 模式，避免 GFM overlay 把 scheme: / @host 误标为 cm-link
    ;(cm as CodeMirrorInstance & { setOption: (option: string, value: object) => void }).setOption(
      'mode',
      {
        name: 'markdown',
  ...
  const insertText = useCallback((before: string, after: string) => {
    ...
    if (cm) {
      ...
      const to = clampCursor(doc, cm.getCursor('to'))
      ...
      cm.operation(() => {
        ...
        if (selection) {
          ...
          // 用 from/to 替换选区；先 setCursor 会折叠选区导致原文残留、内容重复
          cm.replaceRange(before + selection + after, from, to)
        ...
        const cursor = clampCursor(doc, cm.getCursor())
        cm.setCursor(cursor)
  ...
  const options = useMemo(() => {
    return {
      ...
      // 仅编辑；详情页统一走 react-markdown，禁用 EasyMDE 预览快捷键
      shortcuts: {
        togglePreview: null,
  ...
  useEffect(() => {
    ...
    wrapper.addEventListener('paste', handlePasteEvent, true)

    return () => {
      wrapper.removeEventListener('paste', handlePasteEvent, true)
    }
  }, [cmReady, onChange])
  ...
  return (
    <div className="notes-editor-container">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
            ...
            <input
              ...
              style={{
                ...
                border:
                  '2px solid color-mix(in srgb, var(--theme-accent, #2563eb) 60%, transparent)',
                borderRadius: '6px',
                padding: '6px 10px',
                fontSize: '12px',
                ...
                boxShadow:
                  '0 0 0 1px color-mix(in srgb, var(--theme-accent, #2563eb) 20%, transparent)',
              }}
              onFocus={(e) => {
                e.target.style.border =
                  '2px solid color-mix(in srgb, var(--theme-accent, #2563eb) 80%, transparent)'
                e.target.style.boxShadow =
                  '0 0 0 2px color-mix(in srgb, var(--theme-accent, #2563eb) 20%, transparent)'
              }}
              onBlur={(e) => {
                e.target.style.border =
                  '2px solid color-mix(in srgb, var(--theme-accent, #2563eb) 60%, transparent)'
            ...
            <button
              onClick={onAddTag}
              style={{
                background: 'color-mix(in srgb, var(--theme-accent, #2563eb) 20%, transparent)',
                border:
                  '2px solid color-mix(in srgb, var(--theme-accent, #2563eb) 60%, transparent)',
                borderRadius: '6px',
                padding: '6px 10px',
                ...
                textAlign: 'center',
                boxShadow:
                  '0 0 0 1px color-mix(in srgb, var(--theme-accent, #2563eb) 10%, transparent)',
              }}
            >
              +
            </button>
            ...
            <>
              {tags.map((tag, index) => (
                <span
                  key={index}
                  style={{
                    background: 'color-mix(in srgb, var(--theme-accent, #2563eb) 10%, transparent)',
                    color: 'var(--theme-accent, #2563eb)',
                    border:
                      '1px solid color-mix(in srgb, var(--theme-accent, #2563eb) 20%, transparent)',
                    borderRadius: '12px',
                    padding: '2px 8px',
                    fontSize: '12px',
                    display: 'flex',
      ...
      <style>{`
        ...
        .notes-editor-container .CodeMirror-wrap .CodeMirror-scroll { overflow-x: auto !important; }
