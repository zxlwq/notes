...
const List: React.FC = () => {
  ...
  const [logoUrl, setLogoUrl] = useState('/favicon.ico')
    ...
    const loadSettingsTitle = () => {
      try {
        ...
        if (saved) {
          ...
          const nextLogo = typeof parsed.logoUrl === 'string' ? parsed.logoUrl.trim() : ''
          setLogoUrl(nextLogo || '/favicon.ico')
        }
      } catch {}
    }
    loadSettingsTitle()
  ...
  useEffect(() => {
    const settingsSavedHandler = () => {
      showAlert('外观设置已保存并生效！', {
        type: 'success',
        title: '设置成功',
        confirmText: '确定',
      })
    }
    window.addEventListener('settings-save-success', settingsSavedHandler)
    return () => window.removeEventListener('settings-save-success', settingsSavedHandler)
  }, [showAlert])
        ...
        <div className="flex h-16 items-center gap-2 px-4 sm:gap-3 sm:px-6 lg:gap-4 lg:px-8">
          <div className="flex min-w-0 shrink-0 items-center gap-2">
            <img
              ...
              onError={() => {
                if (logoUrl !== '/favicon.ico') setLogoUrl('/favicon.ico')
              }}
            />
            <h1
              className="max-w-28 truncate font-semibold text-gray-900 sm:max-w-40 md:max-w-none"
              style={{ fontSize: '1rem' }}
            >
              {displayTitle || '笔记系统'}
            </h1>
          ...
          <div className="flex min-w-0 flex-1 items-center justify-center px-1">
            {flash && (
              <button
                ...
                className={`inline-flex min-w-0 max-w-full items-center truncate rounded-full border px-3 py-1.5 text-sm font-medium transition-colors hover:opacity-90 sm:px-4 ${flash.action === 'created' ? 'border-green-200/80 bg-green-50/80 text-green-700' : 'border-blue-200/80 bg-blue-50/80 text-blue-700'}`}
          ...
          <div className="flex min-w-0 shrink-0 items-center justify-end gap-2 sm:gap-3 md:gap-4">
            <div className="relative w-[min(100%,12rem)] min-w-0 sm:w-48 md:w-72 lg:w-80">
              <AdvancedSearch
                notes={notes}
      ...
      <main className="w-full">
        ...
        <div className="flex gap-6">
          <div className="-ml-4 w-80 flex-shrink-0 sm:-ml-6 lg:-ml-8">
            <div className="ml-4 min-w-0 overflow-hidden rounded-lg border border-white/30 bg-white/40 p-4 shadow backdrop-blur-lg sm:ml-6 lg:ml-8">
              <h3
              ...
              <div className="space-y-0">
                {getAllTags.map((tag, index) => {
                  return (
                    <div key={tag}>
                      {index > 0 && <div className="my-1 border-t border-white/30" aria-hidden />}
                      <button
                        onClick={() => handleTagClick(tag)}
                        className="flex w-full min-w-0 items-center overflow-hidden rounded-lg px-3 py-2 text-left text-xl font-semibold"
                        style={{
                          backgroundColor: 'transparent',
                          color: 'var(--theme-tag, #fff)',
                          transition: 'none',
                        }}
                        draggable
                        ...
                        onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => {
                          ...
                          e.currentTarget.style.color = 'var(--theme-tag, #fff)'
                        }}
                      >
          ...
          <div className="min-w-0 flex-1 p-4 sm:px-6 lg:px-8">
            {error && (
            ...
            {filteredNotes.length > 0 ? (
              ...
              <div className="py-12 text-center">
                ...
                <h3 className="mb-2 font-medium text-gray-900" style={{ fontSize: '1rem' }}>
                  {loading
                    ? '正在加载笔记...'
                    : notes.length === 0
                      ? '还没有笔记'
