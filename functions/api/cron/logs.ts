...
import { ensureSharesTable, revokeExpiredShares } from '../../../shared/d1-shares.js'
...
async function pruneLocalD1(env: {
  ...
  await ensureSharesTable(db)
  const [deleted, revokedShares] = await Promise.all([
    pruneOldLogsD1(db, days),
    revokeExpiredShares(db),
  ])
  return new Response(
    JSON.stringify({ success: true, deleted, revokedShares, retentionDays: days }),
    {
      headers: { 'content-type': 'application/json; charset=utf-8' },
    }
  )
