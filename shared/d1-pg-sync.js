...
/**
 * Upsert individual settings rows into PostgreSQL (e.g. password credentials).
 * @param {PgQueryable} pool
 * @param {Array<Record<string, string | null>>} rows
 */
export async function upsertPgSettings(pool, rows) {
...
export async function syncD1SnapshotToPg(pool, snapshot) {
  ...
  const shareRows = snapshot.noteShares ?? []
  ...
  for (const row of shareRows) {
    ...
    await pool.query(
      ...
      [
        ...
        row.tags ?? '[]',
        normalizePgTimestamp(row.created_at),
        row.expires_at ? normalizePgTimestamp(row.expires_at) : null,
        row.revoked_at ? normalizePgTimestamp(row.revoked_at) : null,
      ]
    )
  }

  if (shareTokens.length === 0) {
    await pool.query('DELETE FROM note_shares')
  } else {
    await pool.query('DELETE FROM note_shares WHERE NOT (token = ANY($1::text[]))', [shareTokens])
  }
