...
export const Modal: React.FC<ModalProps> = ({
  ...
  const getIcon = () => {
    ...
    switch (type) {
      case 'success':
        return (
          <CheckCircle className={iconClass} style={{ color: 'var(--theme-accent, #16a34a)' }} />
        )
      case 'warning':
      ...
      default:
        return <Info className={iconClass} style={{ color: 'var(--theme-accent, #2563eb)' }} />
    }
  }
  ...
  const getHeaderStyle = (): React.CSSProperties => {
    switch (type) {
      case 'warning':
        return {
          background: 'color-mix(in srgb, #eab308 16%, var(--theme-glass-solid, #fff))',
          borderBottomColor: 'color-mix(in srgb, #eab308 32%, var(--theme-line, transparent))',
        }
      case 'error':
        return {
          background: 'color-mix(in srgb, #ef4444 14%, var(--theme-glass-solid, #fff))',
          borderBottomColor: 'color-mix(in srgb, #ef4444 28%, var(--theme-line, transparent))',
        }
      case 'success':
      default:
        return {
          background:
            'color-mix(in srgb, var(--theme-accent, #2563eb) 14%, var(--theme-glass-solid, #fff))',
          borderBottomColor:
            'color-mix(in srgb, var(--theme-accent, #2563eb) 28%, var(--theme-line, transparent))',
        }
    }
  }

  // Portal 到 body，避免挂在可点击父节点（如笔记卡片）内时事件冒泡触发跳转
  return createPortal(
    <div
      className="pointer-events-auto fixed inset-0 z-[10000] flex items-center justify-center"
    ...
    >
      ...
      <div
        ...
        tabIndex={-1}
        className="relative mx-4 w-full max-w-md rounded-lg border border-white/50 bg-white/50 shadow-xl outline-none backdrop-blur-xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div
