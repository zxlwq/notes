...
import { signSessionToken, getSessionTtlSec, buildSessionCookie } from '../_utils/session'
import { getEffectivePassword, getAdminPasswordRecord } from '../_utils/auth'
import {
  verifyPassword,
...
import { syncCredentialsToPostgres } from '../_utils/pgSync'
...
export const onRequestPost: PagesFunction = async (context) => {
  ...
  try {
    ...
    if (!storedCredential && !envPassword) {
    ...
    if (await matchesAdminPassword(password, envPassword, record.dbStored, record.passwordSet)) {
      ...
      if (matchedDb && env.NOTESD && record.dbStored && !isPasswordHash(record.dbStored)) {
        await rehashLegacyPassword(env.NOTESD, password, record.dbStored)
        try {
          await syncCredentialsToPostgres(env)
        } catch (err) {
          console.error('[pg-sync] credential sync failed after login rehash:', err)
        }
      }
      ...
      return Response.json(
        ...
        {
          headers: apiCors(request, env, {
            'Set-Cookie': buildSessionCookie(token, isProduction, getSessionTtlSec(env)),
          }),
        }
      )
    }
