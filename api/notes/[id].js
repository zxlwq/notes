...
import { ensureSharesTable, revokeSharesByNoteId } from '../../shared/pg-shares.js'
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      const updated = await updateNote(pool, id, { title, content, tags })
      if (!updated) {
        return res.status(404).json({ success: false, error: 'Note not found' })
      }
    ...
    if (req.method === 'DELETE') {
      ...
      await revokeSharesByNoteId(pool, id)
