...
const Login = () => {
  ...
  const { login, loading: authLoading, needsUnlock } = useAuth()
  const navigate = useNavigate()
  const location = useLocation()
  const isUnlockMode =
    needsUnlock || Boolean((location.state as { unlock?: boolean } | null)?.unlock)
          ...
          <div className="text-center">
            <div className="flex justify-center">
              <img
                ...
                src="/favicon.ico"
                alt="Logo"
                className="login-logo-glow mx-auto size-20 cursor-pointer rounded-full border-2 object-cover transition-all duration-300 ease-in-out hover:scale-110"
                style={{ borderColor: 'var(--theme-accent, #3b82f6)' }}
                loading="eager"
                decoding="async"
              />
            </div>
            <h2 className="mt-4 font-bold text-gray-900" style={{ fontSize: '1.5rem' }}>
              {isUnlockMode ? '登录' : displayTitle}
            </h2>
            {isUnlockMode && <p className="mt-2 text-sm text-gray-600">请输入密码</p>}
          </div>
