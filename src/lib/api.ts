...
function assertRemoteSaveResponse(
  ...
  if (!data || typeof data !== 'object') {
    throw new Error('服务器保存失败：响应无效')
  }
  // 必须显式 success === true，避免空对象 / 非 JSON 被当成成功
  if (data.success !== true) {
    throw new Error(data.error || '服务器保存失败')
  }
  if (options?.requireId && (typeof data.id !== 'string' || !data.id)) {
    throw new Error('服务器未返回笔记 ID')
  }
}

/** 写入后再读一次，确认已落库，避免接口假成功 */
async function verifyRemoteNoteSaved(id: string): Promise<void> {
  const response = await api.get<Note & ApiResponse>(`/api/notes/${id}`)
  if (response.status < 200 || response.status >= 300) {
    throw new Error('保存校验失败：无法从服务器读取笔记')
  }
  const data = response.data
  if (!data || typeof data !== 'object') {
    throw new Error('保存校验失败：服务器响应无效')
  }
  if (data.success === false) {
    throw new Error(data.error || '保存校验失败：笔记未写入数据库')
  }
  if (typeof data.id === 'string' && data.id !== id) {
    throw new Error('保存校验失败：笔记 ID 不匹配')
  }
  if (!('title' in data) && !('content' in data)) {
    throw new Error('保存校验失败：笔记未写入数据库')
  }
}
...
export const notesApi = {
  ...
  createNote: async (
    ...
    if (options?.remoteOnly) {
      ...
      if (!savedId) throw new Error('服务器未返回笔记 ID')
      await verifyRemoteNoteSaved(savedId)
      const full = buildFull()
      full.id = savedId
  ...
  updateNote: async (
    ...
    if (options?.remoteOnly) {
      ...
      await verifyRemoteNoteSaved(id)
...
export const settingsApi = {
  get: (key: string): Promise<AxiosResponse<{ success: boolean; data: unknown }>> =>
    api.get(`/api/settings/${key}`),

  save: (key: string, data: unknown): Promise<AxiosResponse<ApiResponse>> =>
    api.post(`/api/settings/${key}`, data),
}

export const shareApi = {
  create: (body: ShareCreateRequest): Promise<AxiosResponse<ShareCreateResponse>> =>
    api.post('/api/share', body),

  get: (token: string): Promise<AxiosResponse<PublicShare>> => api.get(`/api/share/${token}`),

  revoke: (token: string): Promise<AxiosResponse<ApiResponse>> => api.delete(`/api/share/${token}`),

  listByNote: (noteId: string): Promise<AxiosResponse<{ items: ShareListItem[] }>> =>
